...
from bidavideo.evaluation.utils.eval_utils import depth_to_pcd
...
def visualize_batch(
    ...
    if preds.depth_map is not None:
        ...
        for i in range(len(batch_dict["stereo_video"])):
            # if hasattr(preds, 'RTK'):
            #     R = torch.from_numpy(preds.RTK[0])[None].to(device)
            #     T = torch.from_numpy(preds.RTK[1])[None].to(device)
            #     K = torch.from_numpy(preds.RTK[2])[None].to(device)
            #     print('R', R)
            #     print('T', T)
            #     print('K', K)
            #     print('------------------------------------------')
        ...
        # for mode in ["angle_0", "changing_angle"]:
            res = []
